...
static const char *bash_args[]       = {"bash", "-euc", "{CODE}", "--"};
...
const struct Executor *get_executor(const char *lang) {
    for (CustomExecutor *e = custom_executors; e; e = e->next) {
        if (strcmp(e->executor->lang, lang) == 0) {
            return e->executor;
        }
    }

    for (size_t i = 0; i < sizeof(executors) / sizeof(executors[0]); i++) {
        if (strcmp(executors[i].lang, lang) == 0) {
            return &executors[i];
        }
    }
    ...
    switch (type) {
        ...
        case MD_BLOCK_CODE:
            if (detail && data->last && data->content) {
                ...
                char                 *lang     = substr((char *)c_detail->lang.text, 0, c_detail->lang.size);

                // printf("Node: %s, content: %s\n", data->last->text, data->content);

                CODE_BLOCK *new_code = malloc(sizeof(CODE_BLOCK));
                new_code->next       = NULL;
                new_code->info       = info ? info : "";
                new_code->lang       = lang ? lang : "";
                ...
                // Mark all current and parent as used.
                if (data->last->code_block && get_executor(data->last->code_block->lang)) {
                    for (MD_NODE *node = data->last; node && !node->used; node = node->parent) {
                        node->used = 1;
                    }
                }
            }
        ...
        case MD_BLOCK_P:
            ...
            if (data->last && !data->last->code_block && data->content && !data->last->description) {
...
int exec_node(MD_NODE *node, char **args, int num_args) {
    ...
    int      stack_size   = 0;
    MD_NODE *current_node = node;
    while (current_node) {
        stack[stack_size++] = current_node;
        current_node        = current_node->parent;
    }

    CODE_BLOCK *code_block = node->code_block;
    while (code_block) {
        if (code_block->info && code_block->code) {
            const struct Executor *executor = get_executor(code_block->lang);

            if (!executor) {
                error("%s: Unsupported language: %s\n", config.program, code_block->lang);
                return EXIT_FAILURE;
            }
            ...
            if (pid == 0) {
                ...
                // Change directory
                char *cmd_dir = dirname(config.file_path);
                if (chdir(cmd_dir) != 0) {
                    perror("chdir failed");
                    return 1;
                }
                ...
                size_t arg_idx = 0;
                for (; arg_idx < executor->prefix_args_count; arg_idx++) {
                    // printf("prefix[%ld]=%s\n", i, executor->prefix_args[i]);
                    exec_args[arg_idx] = (char *)executor->prefix_args[arg_idx];
                    if (strstr(exec_args[arg_idx], "{LANG}")) {
                        exec_args[arg_idx] = str_replace_all(executor->prefix_args[arg_idx], "{LANG}", code_block->lang);
                    }
                    if (strstr(exec_args[arg_idx], "{CODE}")) {
                        exec_args[arg_idx] = str_replace_all(executor->prefix_args[arg_idx], "{CODE}", code_block->code);
                    }
                }
...
void show_help() {
    printf("Usage: %s [OPTIONS] [HEADING] [ARGS...]\n"
           ...
           "  -c, --code              Print code block\n"
           "  -1                      List one command per line\n"
           "  -t, --tree              Print tree with description\n"
