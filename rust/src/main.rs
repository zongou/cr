...
impl App {
    ...
    fn parse_custom_executors(&mut self) {
        for (k, v) in env::vars() {
            if let Some(rest) = k.strip_prefix("MD_") {
                ...
                let parts = v.split(',').map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    fn parse_file(&self, path: &Path) -> Result<Vec<MDNode>> {
        ...
        // First pass: collect headings and build tree structure
        let mut nodes: Vec<MDNode> = Vec::new();
        // Stack of mutable pointers to last node at each open level
        ...
        // Helper to find and attach to a heading by text (case-insensitive)
        fn attach_code(nodes: &mut [MDNode], heading: &str, lang: String, code: String) -> bool {
            for node in nodes.iter_mut() {
                if node.text.eq_ignore_ascii_case(heading) {
                    node.code_blocks.push(CodeBlock { lang, code });
                    return true;
                }
        ...
        // Second pass: collect code blocks and paragraphs, attach to the last seen heading text
        let parser2 = MdParser::new_ext(&src, options);
    ...
    fn print_tree(&self, nodes: &[MDNode]) {
        // After attaching code blocks and descriptions, mark nodes used if they or their descendants have executable code blocks
        fn mark_used(nodes: &mut [MDNode], app: &App) -> bool {
            let mut any_used = false;
            for node in nodes.iter_mut() {
                let mut used = false;
                // If any code block has an executor, mark
                for cb in &node.code_blocks {
                    if app.get_executor(cb.lang.as_str()).is_some() {
                        used = true;
                        break;
                    }
                }
                ...
                if !node.children.is_empty() {
                    if mark_used(&mut node.children, app) {
                        used = true;
                    }
                }
                node.used = used;
        ...
        // Expose printing that mirrors the Go ascii tree with aligned descriptions
        ...
        // mark used on provided nodes
        let mut nodes_owned = nodes.to_vec();
        // We need to mutate usage flags, so work on owned copy
        mark_used(&mut nodes_owned, self);
        for node in &nodes_owned {
            if node.used {
                // compute max branch for this root only (matches Go behavior)
                let max_branch = compute_max_branch_width_for(node);
                // print root
                println!("{}", node.text);
                print_subtree(node, self, "", true, max_branch);
            }
        }
    }
    ...
    fn exec_node(&self, node: &MDNode, origin_args: &[String], file_path: &Path) -> i32 {
        ...
        for cb in &node.code_blocks {
            ...
            let exe = self.get_executor(lang);
            if exe.is_none() {
