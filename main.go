...
func (r *MDNodeParser) LeaveBlock(t ast.BlockType, detail any) error {
	...
	case ast.BlockCode:
		...
		if r.last != nil {
			...
			// Mark all current and parent node as used.
			if r.last.codeBlock != nil && getExecutor(string(r.last.codeBlock.detail.Lang.Text)) != nil {
				for node := r.last; node != nil && !node.used; node = node.parent {
					node.used = true
				}
			}
		}
	...
	// case ast.BlockHTML:
	case ast.BlockP:
		if r.last != nil && r.last.codeBlock == nil && r.content != "" && r.last.description == "" {
			r.last.description = string(r.content)
		}
	// case ast.BlockTable:
	// case ast.BlockTHead:
	// case ast.BlockTBody:
...
func nodeToTreeWithDesc(node *MDNode) treeprint.Tree {
	...
	walk = func(node *MDNode, parent *treeprint.Tree) {
		for currentNode := node.child; currentNode != nil; currentNode = currentNode.next {
			if currentNode.used {
				branchVal := strings.ToLower(currentNode.text) + " " + strings.Repeat(" ", maxBranchWidth-(currentNode.headingDetail.Level-1)*branchSymbolWidth-runewidth.StringWidth(currentNode.text)) + " " + currentNode.description
...
func execNode(node *MDNode, originArgs []string) int {
	...
	for currentCodeBlock := node.codeBlock; currentCodeBlock != nil; currentCodeBlock = currentCodeBlock.next {
		...
		for i, arg := range executor {
			prefixArgs[i] = arg
			if strings.Contains(prefixArgs[i], "{LANG}") {
				prefixArgs[i] = strings.ReplaceAll(arg, "{LANG}", string(currentCodeBlock.detail.Lang.Text))
			}
			if strings.Contains(prefixArgs[i], "{CODE}") {
		...
		cmd.Dir = filepath.Dir(config.filePath)
	...
	if argi < argsCount {
		...
		if config.one {
			...
			for currentNode := docNode; currentNode != nil; currentNode = currentNode.next {
				currentTree := nodeToTreeWithDesc(currentNode)
				if currentTree != nil {
					fmt.Print(currentTree.String())
				}
			}
		}
	}
