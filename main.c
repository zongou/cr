...
static const struct language_profile language_configs[] = {
    {"sh", sh_args, 4},
    {"bash", sh_args, 4},
    {"zsh", sh_args, 4},
    {"fish", sh_args, 4},
...
const struct language_profile *get_language_config(const char *lang) {
    const struct language_profile *config = NULL;
    // Find language configuration
    for (size_t i = 0; i < sizeof(language_configs) / sizeof(language_configs[0]);
         i++) {
...
TABLE *new_table(unsigned col_count, unsigned head_row_count,
                 unsigned body_row_count) {
...
void md_print_table(TABLE *table) {
    ...
    printf("Table: %d columns, %d header rows, %d body rows\n", table->col_count,
           table->head_row_count, table->body_row_count);
    ...
    switch (type) {
        ...
        case MD_BLOCK_TABLE:
            if (detail) {
                ...
                data->table =
                    new_table(d->col_count, d->head_row_count, d->body_row_count);
            }
            break;
        case MD_BLOCK_TH:
            if (detail) {
    ...
    switch (type) {
        ...
        case MD_BLOCK_CODE:
            if (detail && data->last && data->content) {
                ...
                if (info) {
                    const struct language_profile *lang_config = get_language_config(info);
        ...
        case MD_BLOCK_TABLE: {
            ...
            if (table->head_row_count == 1 && table->body_row_count > 0) {
                if (strcmp("key", table->head[0][0]) == 0 &&
                    strcmp("value", table->head[0][1]) == 0) {
                    ENV_ENTRY *last = NULL;
                    for (int i = 0; i < table->body_row_count; i++) {
                        ENV_ENTRY *new_env = malloc(sizeof(ENV_ENTRY));
        ...
        case MD_BLOCK_H: {
            ...
            new_node->text = data->content ? strdup(data->content) : NULL;
        ...
        case MD_BLOCK_TH:
            ...
            // data->content);
            data->table->head[data->row_index][data->cell_index] =
                data->content == NULL ? NULL : strdup(data->content);
            data->cell_index++;
            break;
        case MD_BLOCK_TD:
            // printf("tb: %d%d %s\n", data->row_index, data->cell_index,
            // data->content);
            data->table->body[data->row_index][data->cell_index] =
                data->content == NULL ? NULL : strdup(data->content);
            data->cell_index++;
            break;
    }
...
Tree *md_to_command_tree(MD_NODE *head, Tree *parent) {
    ...
    while (current != NULL) {
        if (current->code_block || current->child) {
            ...
            Tree *current_tree = new_tree(current->text ? current->text : "");
            add_subtree(parent, current_tree);
...
Tree *md_to_command_tree2(MD_NODE *head, Tree *parent, int max_len) {
    ...
    while (current != NULL) {
        if (current->code_block || current->child) {
            ...
            size_t text_len    = strlen(current->text ? current->text : "");
...
MD_NODE *recursive_find_node(MD_NODE *root, char *target) {
    MD_NODE *current = root;
    while (current) {
        info("current=%s\n", current->text);
        if (current->text && strcasecmp(current->text, target) == 0) {
            return current;
        }
        if (current->child) {
            MD_NODE *result = recursive_find_node(current->child, target);
            if (result) {
                return result;
            }
        }
...
MD_NODE *md_find_node(MD_NODE *root, char *node_path) {
    if (root == NULL || node_path == NULL) {
        return NULL;
    }

    // Check if the node path contains slashes
    char *slash_pos = strchr(node_path, '/');
    if (!slash_pos) {
        MD_NODE *h1_node = root;
        while (h1_node) {
            info("root=%s\n", h1_node->text);

            MD_NODE *current = h1_node->child;
            MD_NODE *result  = recursive_find_node(current, node_path);
            if (result) {
                return result;
            }

            h1_node = h1_node->next;
        }
    }

    MD_NODE *h1_node        = root;
    char    *node_path_copy = strdup(node_path);
    if (!node_path_copy) {
        return NULL; // Memory allocation failed
    }
    char *token = strtok(node_path_copy, "/");
    ...
    while (h1_node) {
        ...
        MD_NODE *current = h1_node->child;
        while (current) {
            info("current=%s, target=%s\n", current->text, token);
            if (current->text && token && strcasecmp(current->text, token) == 0) {
                token = strtok(NULL, "/");
                info("found=%s, next_target=%s\n", current->text, token);
...
int execute_node(MD_NODE *node, char **args, int num_args) {
    ...
    while (block) {
        if (block->info && block->content) {
            ...
            const struct language_profile *lang_prof = get_language_config(lang);

            if (lang_prof) {
                info("Executing code block: \n```%s\n%s```\n", block->info,
                     block->content);
                info("Using language profile: %s\n", lang_prof->name);
                ...
                if (pid == 0) {
                    ...
                    int total_args = lang_prof->prefix_args_count; // Prefix arguments
                    if (num_args > 0)
                    ...
                    int arg_idx = 0;
                    for (size_t i = 0; i < lang_prof->prefix_args_count; i++) {
                        if (strcmp(lang_prof->prefix_args[i], "$CODE") == 0) {
                            exec_args[arg_idx++] = block->content;
                        } else if (strcmp(lang_prof->prefix_args[i], "$NAME") == 0) {
                            exec_args[arg_idx++] = (char *)lang_prof->name;
                        } else {
                            exec_args[arg_idx++] = (char *)lang_prof->prefix_args[i];
                        }
                    }
...
int print_node_env(MD_NODE *root, const char *key) {
    ...
    error("Environment variable not found: %s\n", key);
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    while (argi < argc) {
        ...
        if (current_arg_len > 1 && current_arg[0] == '-') { // Is an option
            if (current_arg[1] != '-') {                    // Is an short options
                ...
                } else if (strcmp(current_arg, "--key") == 0 && argi < argc - 1) { // Pattern: --key **
                    argi++;
                    if (argv[argi]) {
                        config.key = argv[argi];
                    }
    ...
    info("flags: verbose=%d, help=%d, all=%d, markdown=%d, code=%d, file_path=%s, key=%s\n",
         config.verbose, config.help, config.all, config.markdown, config.code, config.file_path, config.key);
    ...
    MD_NODE *doc = md_parse_file(config.file_path);
    ...
    if (argi < argc) {
        ...
        char  *node_path = argv[argi];
        char **cmd_args  = argv + argi + 1;
        int    num_args  = argc - argi - 1;

        info("Looking for node_path '%s'\n", node_path);
        ...
        if (doc) {
            node_found = md_find_node(doc, node_path);
        }
