...
int execute_node(MD_NODE *node, char **args, int num_args) {
    ...
    while (block) {
        if (block->info && block->content) {
            ...
            if (config) {
                info_msg("Executing code block: \n```%s\n%s```\n", block->info, block->content);
                info_msg("Using language config: %s\n", config->name);
                ...
                if (pid == 0) {
                    ...
                    if (!WIFEXITED(status) || exit_code != 0) {
                        info_msg("Command failed with status %d\n", exit_code);
                    } else {
                        info_msg("Command completed successfully %d\n", exit_code);
                    }
                }
            } else {
                error_msg("Unsupported language: %s\n", lang);
                return 1;
            }
        }
        if (exit_code) {
...
MD_NODE *md_parse_file(char *file_path) {
    ...
    if (!fp) {
        error_msg("Cannot open README.md\n");
        return NULL;
    }
    ...
    if (result != 0) {
        error_msg("Error: Markdown parsing failed with code %d\n", result);
        // } else {
        //     info_msg("Parsing completed successfully\n");
    }
...
Tree *md_to_command_tree2(MD_NODE *head, Tree *parent, int max_len) {
    ...
    while (current != NULL) {
        if (current->code_block || current->child) {
            ...
            int    space_count = max_len - (int)text_len - (current->level - 1) * TREE_ITEM_RUNE_LENGTH;
